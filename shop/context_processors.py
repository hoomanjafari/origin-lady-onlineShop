...
def cart_detail(request):
    if request.user.is_authenticated:
        ...
        cart_items_count = AddCart.objects.filter(customer=request.user)
        quantity = 0
        total_price = 0
        for i in cart_items_count:
        ...
        cart_items_count = AddCart.objects.filter(guest_session_id=request.session.session_key)
        quantity = 0
        total_price = 0
        for i in cart_items_count:
            quantity += int(i.selected_quantity)
